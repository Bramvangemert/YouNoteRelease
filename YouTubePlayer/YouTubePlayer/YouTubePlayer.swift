...
public func playlistIDFromYouTubeURL(playListURL: NSURL) ->String?{
    if let pathComponents = playListURL.pathComponents where pathComponents.count > 1 && pathComponents[2]=="www.youtube.com" {
        print( pathComponents[3].stringByReplacingOccurrencesOfString("playlist?list=", withString: "", options: NSStringCompareOptions.LiteralSearch, range: nil))
        return pathComponents[3].stringByReplacingOccurrencesOfString("playlist?list=", withString: "", options: NSStringCompareOptions.LiteralSearch, range: nil)

    }
    return playListURL.queryStringComponents()["list"] as? String
}
public func videoIDFromYouTubeURL(videoURL: NSURL) -> String? {
    if let pathComponents = videoURL.pathComponents where pathComponents.count > 1 && pathComponents[2]=="www.youtube.com" {
        return pathComponents[3].stringByReplacingOccurrencesOfString("watch?v=", withString: "", options: NSStringCompareOptions.LiteralSearch, range: nil)
    }
...
public class YouTubePlayerView: UIView, UIWebViewDelegate {
    ...
    public func loadPlaylistFromUrl(videoURL: NSURL){
        if let playListID = playlistIDFromYouTubeURL(videoURL){
    ...
    {
        return evaluatePlayerCommand("getVideoData().title")
    }
    public func play() {
    ...
    private func evaluatePlayerCommand(command: String) -> String {
        ...
        let value = webView.stringByEvaluatingJavaScriptFromString(fullCommand)
        return value!;
    }
